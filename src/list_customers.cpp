...
{
    double cost = 0.0;
    bool is_remove_first_ct = false;
    if (cities.at(0) !=  0)
    {   
        cities.insert(cities.begin(),0);
        is_remove_first_ct = true;
    }
...
void ListOfCustomers::CreateTour()
{
...
void ListOfCustomers::SetCity( const int& index, const int& value )
{
	cities.at(index)= value;
}
int ListOfCustomers::GetCity( const int& index )
{
